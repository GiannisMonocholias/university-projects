...
def nearest_location(loc, loc_list, loc_type=None):
    ...
    if loc_type == None:
     if loc in loc_list:
      loc_list.remove(loc)
     minimum = min(loc_list,key=lambda x: distance(loc,x))     
    else:
     loc_list_2 = [x for x in loc_list if type(x) == loc_type]
     if loc in loc_list_2:
      loc_list_2.remove(loc)
     minimum = min(loc_list_2,key=lambda x: distance(loc,x))     
...
def pick_cherries_only():
    """Emfanizei string pou briskontai se fwliiasmenes listes.
...
def all_lc(func, ls):
    ...
    (for 'h while)."""
    """GRAPSTE TON KWDIKA SAS APO KATW."""
    if sum([func(x) for x in ls]) == len(ls):
     return True
    return False
...
def all_hof(func, ls):
    ...
    """GRAPSTE TON KWDIKA SAS APO KATW."""
    newls = list(filter(func,ls))
    return len(newls) == len(ls)
...
def primes_up_to(n):
    ...
    def divisors(y):
     return [x for x in range(1,y+1) if y%x == 0]
